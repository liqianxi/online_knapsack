...
p_min = 5
p_max = 1000
w_upper_bound = 2 # Each item’s weight is very small, this is the upper bound
w_lower_bound = 0.002
item_amount = 1000
...
def experiment(runs):
    ...
    for each_run in range(runs):
        ...
        for each_item in range(item_amount):
            weight = random.uniform(w_lower_bound,w_upper_bound)
            item_weight_list.append(weight)
            #value_lower_bound = p_min*weight
            #value_upper_bound = p_max*weight
            value_lower_bound = p_min*weight
            value_upper_bound = p_max*weight
    ...
    #print(ratio_list)
    plot_runs([i for i in range(runs)], ratio_list,"q2")
